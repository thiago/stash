...
func TestApprovePullRequest(t *testing.T) {
	...
	for testNumber, test := range tests {
		testServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			url := *r.URL
			wantPath := fmt.Sprintf("/rest/api/1.0/projects/%s/repos/%s/pull-requests/%d/approve", test.project, test.slug, test.id)
		...
		err := stashClient.ApprovePullRequest(test.project, test.slug, test.id, test.version)
